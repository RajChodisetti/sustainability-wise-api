...
export async function wattwatchersUserRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', {
    ...
    const users = await db.select(publicColumns).from(wwUsers).where(and(
      isNull(wwUsers.sourceApp),
      isNull(wwUsers.sourceUserId),
    )).orderBy(asc(wwUsers.createdAt));
  ...
  app.get('/:id', {
    ...
    const [user] = await db.select(publicColumns).from(wwUsers).where(and(
      eq(wwUsers.id, id),
      isNull(wwUsers.sourceApp),
      isNull(wwUsers.sourceUserId),
    ));
  ...
  app.patch('/:id', {
    ...
    try {
      const updated = await db.transaction(async (tx) => {
        ...
        const [current] = await tx.select().from(wwUsers).where(and(
          eq(wwUsers.id, id),
          isNull(wwUsers.sourceApp),
          isNull(wwUsers.sourceUserId),
        ));
        ...
        }).from(wwUsers).where(and(
          sql`${wwUsers.role} = 'admin' and ${wwUsers.isActive} = true`,
          isNull(wwUsers.sourceApp),
          isNull(wwUsers.sourceUserId),
        ));
  ...
  app.patch('/:id/password', {
    ...
    const [user] = await db.select().from(wwUsers).where(and(
      eq(wwUsers.id, id),
      isNull(wwUsers.sourceApp),
      isNull(wwUsers.sourceUserId),
    ));
