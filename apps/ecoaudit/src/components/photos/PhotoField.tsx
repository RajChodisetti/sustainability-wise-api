...
export function PhotoField({
  ...
  return (
    <div className="rounded-lg border border-[var(--border)] p-3">
      ...
      {uri ? (
        <div className="mb-2">
          <PhotoThumb uri={uri} label={label} className="mb-0 max-h-48 w-full rounded-lg object-cover" />
        </div>
      ) : (
        <div className="mb-2 flex h-32 items-center justify-center rounded-lg bg-[var(--surface2)] text-sm text-[var(--muted)]">No photo</div>
      )}
      <div className="flex flex-wrap gap-2">
        <label className="cursor-pointer">
          <input type="file" accept="image/*" className="hidden" disabled={disabled || uploading} onChange={(e) => void handleFile(e.target.files?.[0])} />
        ...
        {uri && !disabled ? (
          <Button variant="ghost" className="!px-3 !py-1.5 !text-xs" onClick={() => onChange(null)}>Remove</Button>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        {uris.map((uri, i) => (
          <div key={`${uri}-${i}`} className="relative overflow-hidden rounded-lg border border-[var(--border)]">
            ...
            {!disabled ? (
              <button type="button" className="absolute right-1 top-1 rounded bg-black/60 px-2 py-0.5 text-xs text-white" onClick={() => onChange(uris.filter((_, idx) => idx !== i))}>×</button>
            ) : null}
          </div>
        ))}
        {!disabled ? (
          <label className="flex aspect-square cursor-pointer items-center justify-center rounded-lg border border-dashed border-[var(--border)] bg-[var(--surface2)] text-xs text-[var(--text-sub)]">
            <input type="file" accept="image/*" className="hidden" disabled={uploading} onChange={(e) => void handleAdd(e.target.files?.[0])} />
