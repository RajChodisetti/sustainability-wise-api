...
export function GridSupplyEditor({
  ...
  async function save(event: FormEvent) {
    ...
    if (!draft?.name.trim()) {
      setError('Enter a name for the Grid supply.');
  ...
  async function makeDefault(id: string) {
    ...
    try {
      ...
      onSuccess('Default Grid supply updated.');
    } catch (cause) {
      onError(cause);
    } finally {
      setBusy(false);
  ...
  return (
    <Card className="mb-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="font-extrabold text-[var(--text)]">Grid supplies</h2>
          <p className="mt-1 text-xs leading-5 text-[var(--text-sub)]">Named electrical boundaries used by exact Grid relationships. One active supply is always the default.</p>
        </div>
        <Button variant="secondary" onClick={startAdd} disabled={locked || busy}>
          <Icon name="plus" size={16} />Add Grid supply
        </Button>
      </div>
          ...
          <div key={supply.id} className="flex flex-col gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface2)] p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <p className="font-bold text-[var(--text)]">{supply.name}</p>
                {supply.isDefault ? <span className="rounded-full bg-[var(--green-soft)] px-2 py-1 text-xs font-extrabold text-[var(--green)]">Default</span> : null}
              </div>
              <p className="mt-1 break-all text-xs text-[var(--text-sub)]">
                {supply.nmi ? `NMI ${supply.nmi}` : 'No NMI'} · {supply.externalKey || supply.id}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="secondary" disabled={locked || busy} onClick={() => { setError(''); setDraft(structuredClone(supply)); }}>Edit</Button>
              <Button variant="ghost" className="text-[var(--red)]" disabled={locked || busy} onClick={() => setRemoveId(supply.id)}>Remove</Button>
            </div>
          </div>
        ))}
      </div>
      ...
      {draft ? (
        <form onSubmit={(event) => void save(event)} className="mt-4 rounded-xl border border-[var(--primary)]/30 bg-[var(--primary-soft)] p-4">
          <div className="grid gap-x-4 lg:grid-cols-3">
            <div>
              <FieldLabel htmlFor="grid-supply-name" className="mt-0">Supply name *</FieldLabel>
            ...
            <div>
              <FieldLabel htmlFor="grid-supply-key" className="mt-0">External key (optional)</FieldLabel>
              <Input id="grid-supply-key" value={draft.externalKey || ''} onChange={(event) => setDraft({ ...draft, externalKey: event.target.value })} />
            </div>
          </div>
          <label className="mt-3 flex min-h-11 cursor-pointer items-center gap-3 text-sm font-semibold text-[var(--text)]">
            ...
            Make this the default Grid supply
          </label>
          <div className="mt-3 flex flex-wrap gap-2">
            <Button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Save supply'}</Button>
