...
export function cloudConnectionErrorMessage(error: unknown): string {
  ...
  if (error instanceof NetworkError || /failed to fetch|networkerror|load failed/i.test(message)) {
    return `Cannot reach the API at ${API_DISPLAY_URL}. If you are running locally, restart the dev server so the Next.js proxy is active.`;
  }
