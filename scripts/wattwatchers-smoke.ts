...
import { generateKey, hashPassword } from '../src/auth/apiKey.js';
...
async function main(): Promise<void> {
  ...
  const sourceAdminUsername = `smoke-admin-${suffix}`;
  const sourceAdminPassword = `Smoke-${suffix}-Password!`;
  ...
  try {
    await db.insert(eaUsers).values({
      id: sourceAdminId,
      email: `${sourceAdminUsername}@ecoaudit.users.local`,
      passwordHash: await hashPassword(sourceAdminPassword),
      fullName: 'Smoke Eco administrator',
      role: 'admin',
      isActive: true,
    });
    const bridgedLogin = await app.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: {
        app: 'wattwatchers',
        email: sourceAdminUsername,
        password: sourceAdminPassword,
      },
    });
    assert.equal(bridgedLogin.statusCode, 200, bridgedLogin.body);
    const bridgedAuth = JSON.parse(bridgedLogin.body);
    assert.equal(bridgedAuth.user.app, 'wattwatchers');
    assert.equal(bridgedAuth.user.role, 'admin');
    const bridgedRead = await app.inject({
      method: 'GET',
      url: '/v1/wattwatchers/dashboard/summary',
      headers: { authorization: `Bearer ${bridgedAuth.accessToken}` },
    });
    assert.equal(bridgedRead.statusCode, 200);

    await sql`UPDATE ea_users SET role = 'inspector', updated_at = NOW() WHERE id = ${sourceAdminId}`;
    const revokedSourceRefresh = await app.inject({
      method: 'POST',
      url: '/v1/auth/refresh',
      payload: { refreshToken: bridgedAuth.refreshToken },
    });
    assert.equal(revokedSourceRefresh.statusCode, 401);
    ...
    console.log('[wattwatchers-smoke] migration, source-admin auth, ingestion, partial safety, transitions, CSV, and reads ok');
  } finally {
    await app.close().catch(() => {});
    await sql`DELETE FROM refresh_tokens WHERE user_id IN (
      SELECT id FROM ww_users WHERE source_app = 'ecoaudit' AND source_user_id = ${sourceAdminId}
    )`;
    await sql`DELETE FROM ww_users WHERE source_app = 'ecoaudit' AND source_user_id = ${sourceAdminId}`;
    await sql`DELETE FROM ea_users WHERE id = ${sourceAdminId}`;
